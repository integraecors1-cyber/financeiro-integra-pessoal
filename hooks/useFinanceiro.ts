...
export function useFinanceiro() {
  ...
  const addDespesa = async (item: Despesa) => {
    ...
    if (res.error) {
      console.error('ERRO SUPABASE (addDespesa):', res.error.message);
      setDespesas(previous);
      localStorage.setItem('ic_despesas', JSON.stringify(previous));
    }
  ...
  const addPagamento = async (item: Pagamento) => {
    ...
    if (res.error) {
      console.error('ERRO SUPABASE (addPagamento):', res.error.message);
      setPagamentos(previous);
      localStorage.setItem('ic_pagamentos', JSON.stringify(previous));
    }
    return { error: res.error };
  };
  ...
  const addReceita = async (item: Receita) => {
    ...
    if (res.error) {
      console.error('ERRO SUPABASE (addReceita):', res.error.message);
      setReceitas(previous);
      localStorage.setItem('ic_receitas', JSON.stringify(previous));
    }
    return { error: res.error };
  };
  ...
  return {
    ...
    clearAllData: async () => {
      try {
        ...
        await Promise.all(tables.map(table => supabase.from(table).delete().neq('id', '00000000-0000-0000-0000-000000000000')));
