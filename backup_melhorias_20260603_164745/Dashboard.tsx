...
import { TrendingUp, TrendingDown, Users, Wallet, Receipt, CreditCard, Scale } from 'lucide-react';
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="card group hover:border-gold transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-bold text-[#8A8580] uppercase tracking-widest">{new Date().toLocaleDateString('pt-BR', { month: 'long' })}</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {MEMBROS.map((membro) => {
              const res = calcResumoFinanceiroMembro(finance, membro, currentMonth);

              // Status Cota
              let cotaStatus = 'Pendente';
              let cotaColor = 'text-[#E07070]';
              if (res.saldoCota > 0.01) { cotaStatus = 'Pago a Mais'; cotaColor = 'text-[#7ABF8A]'; }
              else if (res.saldoCota < -0.01) { cotaStatus = 'Pendente'; cotaColor = 'text-[#E07070]'; }
              else { cotaStatus = 'Quitado'; cotaColor = 'text-[#7ABF8A]'; }
