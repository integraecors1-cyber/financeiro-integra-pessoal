...
export default function Config({ finance }: any) {
  ...
  const addCatDespesa = () => {
    ...
    finance.setCatsDespesa((prev: string[]) => [...prev, newCatDesp.trim()]);
    setNewCatDesp('');
  };

  const addCatReceita = () => {
    if (!newCatRec.trim()) return;
    finance.setCatsReceita((prev: string[]) => [...prev, newCatRec.trim()]);
    setNewCatRec('');
  };
  ...
  const addDespesaFixa = () => {
    ...
    finance.setDespesasFixas((prev: any[]) => [...prev, item]);
    setNewFixa({ nome: '', valor: '', cat: '' });
  };

  const registrarFixasNoMes = () => {
    ...
    const novasDespesas = finance.despesasFixas.map((f: any) => ({
      id: uid(),
      data,
      valor: f.valor,
      desc: `${f.nome} — ${mesAno}`,
    ...
    finance.setDespesas((prev: any[]) => [...prev, ...novasDespesas]);
    alert(`${novasDespesas.length} despesas fixas registradas.`);
  };
