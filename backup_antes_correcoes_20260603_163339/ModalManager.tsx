...
function ModalDespesa({ onSave, finance, data }: any) {
  ...
  const handleSave = async () => {
    if (!form.data || !form.valor || !form.desc || !form.cat || !form.natureza || !form.essencialidade || !form.centroCusto) return;
    ...
    if (form.parcelado && form.parcelas > 1) {
      ...
      if (data) {
        await finance.updateDespesa(item);
      } else {
        await finance.addDespesa(item);
      }
    }
  ...
  return (
    <div className="p-10 space-y-8">
      ...
      <div className="pt-4 flex flex-col gap-3">
        <button onClick={handleSave} className="btn btn-primary py-4 text-sm tracking-widest uppercase">Salvar Registro</button>
