...
export default function Despesas({ finance, onEdit, onDelete }: any) {
  ...
  const totalFiltrado = useMemo(() =>
    filteredList.reduce((s: number, d: any) => s + d.valor, 0)
  , [filteredList]);
        ...
        <div className="flex items-center gap-3">
          ...
          {filteredList.length > 0 && (
            <span className="text-[10px] font-bold text-[#8A8580] uppercase tracking-widest">
              {filteredList.length} registro{filteredList.length !== 1 ? 's' : ''} · {fmt(totalFiltrado)}
            </span>
          )}
        </div>
                ...
                <tr key="empty-despesas">
                  <td colSpan={9} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-[#8A8580] text-sm italic">Nenhuma despesa em {labelMes(mesSelecionado)}.</p>
                    </div>
                  </td>
                </tr>
              )}
