...
import { TrendingUp, Users, Receipt, CreditCard, Scale, ChevronLeft, ChevronRight } from 'lucide-react';
import { fmt } from '@/lib/utils';
import { calcResumoFinanceiroMembro } from '@/lib/calculations';
...
export default function Dashboard({ finance, onSelectMember }: any) {
  ...
  const totalReceitas = useMemo(() =>
    finance.receitas.filter((r: any) => !r.deleted && r.data.startsWith(mesSelecionado))
      .reduce((acc: number, r: any) => acc + r.valor, 0)
  , [finance.receitas, mesSelecionado]);

  const totalDespesas = useMemo(() =>
    finance.despesas.filter((d: any) => !d.deleted && d.data.startsWith(mesSelecionado))
      .reduce((acc: number, d: any) => acc + d.valor, 0)
  , [finance.despesas, mesSelecionado]);

  const totalPagamentos = useMemo(() =>
    finance.pagamentos.filter((p: any) => !p.deleted && p.data.startsWith(mesSelecionado))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <h3 className="text-xl font-serif font-bold text-gold flex items-center gap-2">
            <Users size={20} /> Resumo Financeiro por Membro
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {MEMBROS.map((membro) => {
              const res = calcResumoFinanceiroMembro(finance, membro, mesSelecionado);
              let cotaStatus = 'Pendente', cotaColor = 'text-[#E07070]';
              ...
              return (
                <div key={membro} onClick={() => onSelectMember(membro)}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[9px] uppercase text-[#8A8580] tracking-[0.2em] font-bold">Resumo Financeiro</p>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <h3 className="text-xl font-serif font-bold text-gold flex items-center gap-2">
            <Receipt size={20} /> Últimas Operações
          </h3>
          <div className="card p-0 overflow-hidden divide-y divide-[#383838]">
            {[...finance.despesas, ...finance.pagamentos, ...finance.receitas]
              ...
              .slice(0, 6)
              .map((item: any) => (
                <div key={`${item.pagador ? 'pag' : item.origem ? 'rec' : 'desp'}-${item.id}`} className="p-4 flex items-center justify-between hover:bg-[#242424] transition-colors">
                  <div className="flex items-center gap-3">
